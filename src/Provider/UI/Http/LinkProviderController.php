...
use App\Provider\UI\Http\Request\LinkProviderRequest;
...
final class LinkProviderController extends AbstractController
{
    ...
    #[Route('/api/providers/{providerId}/link', name: 'api_provider_link', methods: ['POST'])]
    #[OA\Post(
        path: '/api/providers/{providerId}/link',
        description: 'Links two providers together. Admin role required.',
        summary: 'Link providers',
        security: [['Bearer' => []]],
        tags: ['Provider'],
    )]
    #[OA\Parameter(
    ...
    #[OA\RequestBody(
        required: true,
        content: new OA\JsonContent(ref: new Model(type: LinkProviderRequest::class)),
    )]
    #[OA\Response(
        response: Response::HTTP_CREATED,
        description: 'Providers linked successfully.',
    )]
    #[OA\Response(
    ...
    public function __invoke(string $providerId, Request $request): JsonResponse
    {
        ...
        /** @var LinkProviderRequest $dto */
        $dto = $this->jsonDtoFactory->create($request, LinkProviderRequest::class);
        $this->commandBus->dispatch(new LinkProvider(
            $providerId,
            $dto->linkedProviderId,
            $user->getId(),
        ));
